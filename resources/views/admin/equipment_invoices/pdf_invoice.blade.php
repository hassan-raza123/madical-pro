...
<body class="bg-white">
    ...
    <div class="container-fluid px-4" id="content">
        ...
        <img src="{{ asset($invoice->transaction->company->invoice_bg) }}" class="bg-image">

        {{-- <img src="{{ asset($invoice->transaction->company->header_img) }}" width="100%" alt="Company Image"> --}}
        ...
        <table>
            <thead class="text-center border-bottom">
                <th class="border-right">Vehicle</th>
                <th class="border-right">Operators</th>
                <th class="border-right">Price Type</th>
                <th class="border-right">Unit Price (RO)</th>
                <th class="border-right">Total Hours</th>
                <th class="border-right">Mobilization (RO)</th>
                <th class="border-right">Demobilization (RO)</th>
            ...
            <tbody class="text-center">
                ...
                @endphp
                @foreach ($invoice->equipments as $invoice_equip)
                @php
                    $unit_price = $invoice_equip->unit_price; 
                    $total_hours = $invoice_equip->total_hours;
                    $mob = $invoice_equip->mobilization; 
                    $demob = $invoice_equip->demobilization; 
                    if (!$mob) {
                        $mob = 0;
                    } 
                    if (!$demob) {
                    ...
                    $sub_total = ($unit_price * $total_hours) + $mob + $demob;
                    $total += $sub_total;
                    $row_count++;
                @endphp
                <tr class="border-bottom">
                    <td class="border-right">{{ $invoice_equip->transaction_equipments->equipment->name }} ({{ $invoice_equip->transaction_equipments->equipment->reg_no }})</td>
                    <td class="border-right">
                        @foreach ($invoice_equip->transaction_equipments->operators as $optr)
                            {{ $optr->employee->name }}, 
                        @endforeach
                    </td>
                    <td class="border-right">{{ $invoice_equip->price_type }}</td>
                    <td class="border-right">{{ $unit_price }}</td>
                    <td class="border-right">{{ $total_hours }}</td>
                    <td class="border-right">{{ $mob }}</td>
                    <td class="border-right">{{ $demob }}</td>
                    <td>{{ $sub_total }}</td>
                </tr>
                @endforeach

                @for ($i = 0; $i < 15 - $row_count; $i++)
                <tr class="border-bottom">
                ...
                <tr>
                    <td colspan="5" class="text-left pl-3"><strong>Bank:</strong> {{ $invoice->transaction->company->bank }}</td>
                    <td class="border-left" colspan="2" style="border-top: 1.5px solid #000;">Sub Total:</td>
                    <td class="border-left" style="border-top: 1.5px solid #000;">{{ $total }}</td>
                </tr>
                <tr>
                    <td colspan="5" style="border: none !important;" class="text-left pl-3"><strong>Account:</strong> {{ $invoice->transaction->company->bank_account_no }}</td>
                    <td class="border-left border-top" colspan="2">VAT ({{$vat}}%):</td>
                    @php
                        $vat_amount = $total * $vat/100;
                    @endphp
                    <td class="border-left border-top">{{ round($vat_amount, 2) }}</td>
                </tr>
                <tr>
                    <td colspan="5" style="border: none !important;" class="text-left pl-3"><strong>Branch:</strong> {{ $invoice->transaction->company->bank_branch }}</td>
                    <td class="border-left border-top" colspan="2" rowspan="2">Total:</td>
                    <td class="border-left border-top" rowspan="2">{{ round(($vat_amount + $total), 2) }}</td>
                </tr>
                <tr>
            ...
            <tbody>
                <tr>
                    <td class="border-right py-4">
                        <img src="{{ asset($invoice->transaction->company->signature_img) }}" class="mx-auto d-block" style="height: 70px; width: auto;" alt="Signature Image">
                    </td>
                    <td></td>
                </tr>
            </tbody>
