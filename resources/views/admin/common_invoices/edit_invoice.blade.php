...
@section('title', 'Edit Common Invoice')
@section('heading', 'Edit Common Invoice')
...
<form class="row mb-0" autocomplete="off" method="POST" action="{{ route('common_invoices.update', $invoice->id) }}">
    @csrf
    ...
    <div class="col-sm-6">
        <div class="form-group mb-2">
            ...
            <div class="quot_no" action="action_page.php">
                <input type="number" class="form-control" id="invoice_no" value="{{ $invoice->invoice_no_arr[0] }}" readonly>
                <span class="invoice_no_span" id="invoice_no_span">/{{ $invoice->invoice_no_arr[1] }}</span>
                <input type="hidden" name="invoice_no" id="invoice_no_input" value="{{ $invoice->invoice_no }}">
            </div>
        </div>
    </div>
...
<script>
    let invoice = JSON.parse('{!! $invoice !!}');
    ...
    $(document).ready( () => {
        $('#company_id').on('change', (e) => {
            if (invoice.company_id == e.target.value) {
                $('#invoice_no_input').val(invoice.invoice_no_arr[0]+'/'+invoice.invoice_no_arr[1]);
                $('#invoice_no').val(invoice.invoice_no_arr[0]);
                $('#invoice_no_span').text('/'+invoice.invoice_no_arr[1]);
            } else {
