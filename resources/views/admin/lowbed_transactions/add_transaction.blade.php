...
@section('title', 'Add Lowbed Transaction')
@section('heading', 'Add Lowbed Transaction')

@section('main_content')

<form class="row mb-0" autocomplete="off" method="POST" action="{{ route('lowbed_transactions.store') }}">
    @csrf
        ...
        <div class="form-group">
            ...
            <select name="lowbed_id" class="form-control" required>
                ...
                @foreach ($lowbeds as $lowbed)
                    <option value="{{ $lowbed->id }}">{{ $lowbed->name }} ({{ $lowbed->reg_no }})</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="col-12 mt-4">
        <input type="submit" class="btn btn-primary btn-rounded float-right" value="SAVE TRANSACTION">
